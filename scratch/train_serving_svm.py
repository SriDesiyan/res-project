...
def main():
    ...
    print("Collecting waiter pose features...")
            ...
            for w in waiters:
                ...
                if crop_h > 20 and crop_w > 20:
                    ...
                    if pose_results.pose_landmarks:
                        ...
                        # Check if serving (Method 4, or arm raised with bent elbows)
                        # We run detect_waiter_serving to see if the old heuristics detected it
                        ...
                        # Define robust positive and negative labels
                        is_positive = False
                        is_negative = False
                        
                        # Positive: clear serving pose or plate carrying
                        if is_serving_heuristic:
                            is_positive = True
                        elif (60 < left_angle < 115 and lw.y < le.y) or (60 < right_angle < 115 and rw.y < re.y):
                            # Wrist above elbow, elbow bent
                            # Make sure it is not order taking (hands close)
                            if not (wrist_dist < 0.18 and left_angle < 75 and right_angle < 75):
                                is_positive = True
                                
                        # Negative: idle (arms straight down) or writing notes (wrist close, arms tucked)
    ...
    print(f"Collected {len(features)} total samples.")
    print(f"  Positives (Serving): {np.sum(labels == 1)}")
    print(f"  Negatives (Not Serving): {np.sum(labels == 0)}")
    
    if len(features) < 10:
        print("Not enough features collected. Check video path or ranges.")
        return
